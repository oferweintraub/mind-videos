...
def invalidate_episode_for_character(slug: str) -> int:
    ...
    Called when a character's voice, tempo, or image is edited — the
    skip-if-exists pipeline would otherwise reuse stale audio/video on the
    next render. Returns number of files deleted.

    Scope: only the *current* episode (derived from the in-memory title).
    ...
    for sub in ("audio", "videos"):
        d = episode_dir / sub
        if d.exists():
            # Match seg##_<slug>.{mp3,mp4} AND the _raw.mp3 intermediates
            # generated by generate_tts when atempo is applied.
            patterns = (f"seg*_{slug}.*", f"seg*_{slug}_raw.*")
            seen = set()
            for pat in patterns:
                for p in d.glob(pat):
                    if p in seen:
    ...
    # If anything was invalidated, the concatenated final.mp4 is stale too.
    if n > 0:
        ...
        if final_mp4.exists():
            try:
                ...
                pass
        for p in episode_dir.glob("*_list.txt"):
            try:
                p.unlink()
            except OSError:
                pass
        # Clear the result so the done page doesn't link to a deleted file
