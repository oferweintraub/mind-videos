...
def load(slug: str, root: Optional[Path] = None) -> Character:
    ...
    # Backward compatibility: default to public=True for existing manifests
...
def list_all(root: Optional[Path] = None, public_only: bool = False) -> list[Character]:
    """Load all characters. If public_only=True, filter to public characters only."""
    root = Path(root) if root else CHARACTERS_DIR
    if not root.exists():
        return []
    out = []
