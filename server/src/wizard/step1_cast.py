...
from src.utils import save_thumbnail
...
def _generate_candidates_for_draft():
    ...
    async def _one(i: int, style: str) -> Path:
        ...
        if not use_ref:
            ...
            if not c.google:
                ...
                try:
                    result = await generate_with_ref_nano_banana(
                        ref_path, draft["description"], style, out_path,
                        google_api_key=c.google,
                    )
        ...
        save_thumbnail(result, thumb_path)
        return result
    ...
    for (i, style), r in zip(plan, results):
        ...
        candidates.append({
            "idx": i,
            "style": style,
            "path": str(out_dir / f"option_{i}.png"),
            "thumbnail": str(out_dir / f"option_{i}_thumb.jpg"),
        })
